...
d20 = as.data.frame(as_draws_matrix(fit_15, variable = c("b_V2", "b_V6")))
d20$source =  "Implicit Prior"
d20$precon_size = 20

data.grid <- expand.grid(b_V2 = seq(-2, 2, length.out=1000), b_V6 = seq(-2, 2, length.out=1000))
...
p1 = ggplot() +
  ...
  geom_point(data = bind_rows(d5, d10, d20), aes(x = b_V2, y = b_V6, color = source), alpha = 0.1) +
...
p2 = ggplot(as_tibble(custom_R2(prior_only_fit)), aes(x = R2, after_stat(density))) +
  geom_histogram() +
...
p3 = ggplot(as_tibble(custom_R2(fit_5)), aes(x = R2, after_stat(density))) +
  geom_histogram() +
  coord_cartesian(xlim = c(0, 1)) +
  scale_x_continuous(breaks = c(0, 0.5, 1)) +
  ylab("Density") +
...
p4 = ggplot(as_tibble(custom_R2(fit_7)), aes(x = R2, after_stat(density))) +
  geom_histogram() +
  coord_cartesian(xlim = c(0, 1)) +
  scale_x_continuous(breaks = c(0, 0.5, 1)) +
  ylab("Density") +
...
p5 = ggplot(as_tibble(custom_R2(fit_10)), aes(x = R2, y = after_stat(density))) +
  geom_histogram() +
  coord_cartesian(xlim = c(0, 1)) +
  scale_x_continuous(breaks = c(0, 0.5, 1)) +
  ylab("Density") +
...
ggsave("fig_1.pdf",f1, width = 210, height = (297 / 4)*1.5, units = "mm", useDingbats = TRUE)
```
