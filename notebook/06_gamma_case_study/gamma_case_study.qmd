...
p_vanilla_t1 = plot_ecdf_diff(
  filter(
    ...
    variable %in% c("b_Intercept", "b_age", "b_hip", "b_thigh", "shape", "loglik")
  ),
  max_rank = 1000
  )
  ...
  for (p in seq_along(prior_list))
  {
    ...
    for (i in seq_along(N_PRE_list))
    {
      ...
      for (j in seq_along(ds_gen_list))
      {
        ...
        if (names(prior_list)[[p]] != "flat") {
         if(weighted_loop) {
            bform = bf(y | weights(weight_var) ~ ., center = FALSE)
          } else {
            bform = bf(y ~ ., center = FALSE)
          }
...
scale_ifs_result_df <- subset( scale_ifs_result_df, select = -precon_data)
ifs_result_df <- bind_rows(ifs_result_df, scale_ifs_result_df)
...
#bind_rows(ifs_result_df, readRDS("results.RDS")) |>
  #ifs_result_df |>
  ...
  mutate(
    ...
    precon_likelihood = case_when(
      ...
      precon_likelihood == "full_gamma" ~ "250 Simulated",
      precon_likelihood == "full_bodyfat" ~ "250 Bodyfat"
      ),
  ...
  #geom_hline(yintercept = SBC:::adjust_gamma(N = 1000, L = 1, conf_level = 0.95)) +
